...
data class VoicePreset(
    ...
    val styleDisplay: String
        get() {
            ...
            if (descLower.contains("phong cách tin tức") || descLower.contains("tin tức") || descLower.contains("thời sự")) {
                return "Phong cách tin tức"
            }
            if (descLower.contains("phong cách kể chuyện") || descLower.contains("kể chuyện") || descLower.contains("truyện đêm muộn")) {
...
/**
 * Registry and loader for Vietnamese preset voices.
 * Includes curated defaults (Northern, Central, Southern Vietnamese accents, male, female, storyteller, news anchor).
 */
object VoicePresets {
    ...
    init {
        registerCuratedDefaults()
    }

    private fun registerCuratedDefaults() {
        val defaultEmb = FloatArray(192) { 0.01f }
        val curated = listOf(
            VoicePreset("Ngọc Huyền", "Nữ Miền Bắc · Phong cách kể chuyện dịu dàng", "female", "Bắc", "kể chuyện", defaultEmb.clone(), featured = 1),
            VoicePreset("Ly", "Nữ Miền Bắc · Giọng đọc tự nhiên, trong trẻo", "female", "Bắc", "tu_nhien", defaultEmb.clone(), featured = 2),
            VoicePreset("Bình", "Nam Miền Bắc · Trầm ấm, truyền cảm, phong cách tin tức", "male", "Bắc", "tin_tuc", defaultEmb.clone(), featured = 3),
            VoicePreset("Tuyên", "Nam Miền Bắc · Phong cách đọc truyện kiếm hiệp hào sảng", "male", "Bắc", "kiem_hiep", defaultEmb.clone(), featured = 4),
            VoicePreset("Đoan", "Nữ Miền Nam · Giọng ấm áp, truyền cảm, phong cách kể chuyện", "female", "Nam", "kể chuyện", defaultEmb.clone(), featured = 5),
            VoicePreset("Vĩnh", "Nam Miền Nam · Phong cách tự nhiên, chân thực", "male", "Nam", "tu_nhien", defaultEmb.clone(), featured = 6),
            VoicePreset("Hương Mai", "Nữ Miền Trung · Nhẹ nhàng, sâu lắng, đậm chất thơ", "female", "Trung", "kể chuyện", defaultEmb.clone(), featured = 7),
            VoicePreset("Minh Đức", "Nam Miền Trung · Vang sáng, đĩnh đạc, phát thanh viên", "male", "Trung", "tin_tuc", defaultEmb.clone(), featured = 8)
        )
        for (v in curated) {
            presetsMap[v.name] = v
        }
    ...
    fun getVoice(name: String): VoicePreset? {
        return presetsMap[name] ?: presetsMap[defaultVoiceName] ?: presetsMap.values.firstOrNull()
    }
