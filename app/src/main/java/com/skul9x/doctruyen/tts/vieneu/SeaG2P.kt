 ...
 * in `libsea_g2p_android.so` / `libseag2p.so`.
 */
object SeaG2P {
    ...
    /**
     * Algorithmic rule-based Vietnamese fallback phonemizer for JVM unit test environments
     * where native JNI binaries cannot be dynamically linked.
     */
    internal fun fallbackRulePhonemize(text: String): String {
        // Simple phonetic substitution preserving tones and structures
        val words = text.split(Regex("\\s+"))
        return words.joinToString(" ") { word ->
            normalizeWordPhonemes(word)
        }
    }

    private fun normalizeWordPhonemes(word: String): String {
        var w = word.lowercase()
        // Map common Vietnamese onset clusters to IPA/phonetic representation
        w = w.replace("ph", "f")
            .replace("th", "tʰ")
            .replace("ch", "c")
            .replace("tr", "ʈ")
